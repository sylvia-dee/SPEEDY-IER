...
C  Need to copy tracers to an array with no time dimension to
C  pass it correcly. (Could also change dimension order of TR...)
      do ITR=1,NTR
        do K=1,KX
          do N=1,NX
            do M=1,MX
              TRPASS(M,N,K,ITR) = TR(M,N,K,J1,ITR)
            enddo
          enddo
        enddo
      enddo
...
C        DO 222 M=1,MXNX
C          DIVDT(M,1,K)=DIVDT(M,1,K)-DUMC(M,1,2)
C  222 CONTINUE
        DO N=1,NX
          DO M=1,MX
...
C        DO 22 M=1,MXNX
C          TDT(M,1,K)=TDT(M,1,K)+DUMC(M,1,2)
C   22   CONTINUE
        DO N=1,NX
          DO M=1,MX
...
C          DO 32 M=1,MXNX
C            TRDT(M,1,K,ITR)=TRDT(M,1,K,ITR)+DUMC(M,1,2)
C   32     CONTINUE
          DO N=1,NX
            DO M=1,MX
