...
C sdee test to see if we need double preicision here.

C      REAL*8 TRRAT8(NGP,NLEV,NTR),QA8(NGP,NLEV,NTR)
C      TRRAT8 	= dble(TRRAT)
C      QA8 	= dble(QA)
	...
	RSIG1(K)=1./(1.-SIGH(K))
      ...
      DO ITR=1,NTR
        DO K=1,NLEV
          DO J=1,NGP
            QTENVD(J,K,ITR) = 0.
            TRRAT(J,K,ITR) = QA(J,K,ITR)/(QA(J,K,1)+QTINY)
C            TRRAT8(J,K,ITR) = QA8(J,K,ITR)/(QA8(J,K,1)+QTINY)
          ENDDO
        ENDDO
      ENDDO
