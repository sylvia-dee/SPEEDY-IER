...
C      include "com_anomvar.h"
      ...
      COMPLEX ZERO, CCON, SURFS(MX,NX),FLD2S(MX,NX)
      REAL  SURFG(IX,IL), FLD2G(IX,IL)
...
C       Water isotope tracers at tropospheric levels
        DO ITR=2,NTR
          DO K=3,KX
            DO N=1,NX
             DO M=1,MX
               TR(M,N,K,1,ITR)= Rstd(ITR)*TR(M,N,K,1,1)
             ENDDO
            ENDDO
          ENDDO
        ENDDO
