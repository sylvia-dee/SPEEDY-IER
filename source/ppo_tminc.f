...
C     Logical flags
      ...
      parameter (NSAVE=9)
      real ADSAVE(ngp,NSAVE), FPQ(ngp), PHISG(ngp)
      equivalence (PHISG,PHIS0)

C     fields for vertical interpolation
      integer K0(ngp)
      real    W0(ngp), ZOUT(ngp), ZINP(nlev), RDZINP(nlev),
     &         STLO(ngp), SSTO(ngp), SSTMO(ngp)

      data FACT2D / 2*1000., 2*1., 2*100., 3*1., 100., 100., 1000., 1.,
     &              7*1.0 /
...
c     land and sea surface temperature and
c     ocean model temperature
c
      DO J=1,ngp
        JLAT=(J+NLON-1)/NLON
	J0=J-(JLAT-1)*NLON
          STLO(j)=STL1(j0,jlat)*FMASKL1(j0,jlat)
          SSTO(j)=SST1(j0,jlat)*FMASKO1(j0,jlat)
          IF(FMASKO1(j0,jlat) .EQ. 1.) THEN
            SSTMO(J)=SSTM1(j0,jlat)
          ELSE
            SSTMO(J)=9.999E+19
          ENDIF  
      ENDDO
C
      ...
      call ADD1F (SAVE2D,PRTOP,     ngp,n0)
...
cfk      call ADD1F (SAVE2D,STANOML(1,1),ngp,n0)
cfk      call ADD1F (SAVE2D,SSTAN,        ngp,n0)

      call ADD1F (SAVE2D,STLO,        ngp,n0)
      call ADD1F (SAVE2D,SSTO,        ngp,n0)
      call ADD1F (SAVE2D,SSTMO,        ngp,n0)
...
c         Below the surface, set spec.hum. = near-surface value 
          ...
          do j=1,ngp
            if (ZOUT(j).lt.0.0) then
              ADSAVE(j,5)=Q0(j,1)
              ADSAVE(j,6)=Q0(j,1)/FPQ(j)
            endif
          enddo

        else

          call VERINT (ADSAVE(1,5),QG1(1,1,1),ngp,kx,K0,W0) 

        endif

C       Interpolate the tracers

        call VERINT (ADSAVE(1,7),QG1(1,1,2),ngp,kx,K0,W0) 
        call VERINT (ADSAVE(1,8),QG1(1,1,3),ngp,kx,K0,W0) 
        call VERINT (ADSAVE(1,9),QG1(1,1,4),ngp,kx,K0,W0) 

c       Below the surface, set tracer spec.hum. = near-surface value 
...
C       2.3 Add 3-d fields to time-mean and daily-means arrays
        ...
        if(k.eq.7)then
         do j=1,ngp
           ...
           SAVE3D_D(j,7)=SAVE3D_D(j,7)+ADSAVE(j,7)
           SAVE3D_D(j,8)=SAVE3D_D(j,8)+ADSAVE(j,8)
           SAVE3D_D(j,9)=SAVE3D_D(j,9)+ADSAVE(j,9)
         enddo
        elseif(k.eq.5)then
          do j=1,ngp
...
C--   3. Save diabatic forcing terms on model levels
      ...
      if (iitest.eq.1) print *, 'end of TMINC', n0
