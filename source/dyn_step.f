...
C     3.4 Diffusion of tracers
      ...
      DO K=1,KX
        DO M=1,MX
          DO N=1,NX
C The following for removes origraphic term for consistency with tracers
C           CTMP(M,N,K)=TR(M,N,K,1,1)+QCORH(M,N)*QCORV(K)
...
C-- DCN Degrade Spectral Model from Within...Set Harmonic Coefficients to 0 and truncate at T21.

C      DO K=1,KX
C        DO M=1,MX
C          DO N=1,NX
C             IF (N.gt.21 .or. M.gt.21) THEN
C                 PS(M,N,J2) = (0.0,0.0)
C                 VOR(M,N,J2,K) = (0.0, 0.0)
C                 DIV(M,N,J2,K) = (0.0, 0.0)
C                 CTMP(M,N,K) = (0.0, 0.0)
C                 DO ITR = 1, NTR
C                    TR(M,N,K,J2,ITR) = (0.0, 0.0)
C                 ENDDO
C             ELSE
C             ENDIF
C          ENDDO
